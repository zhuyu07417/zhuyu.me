...
export default function SettingsPanel({ onClose }: { onClose: () => void }) {
  const { bgImage, bgBlur, bgImages, setBgImage, setBgBlur } = useBackground();
  ...
  const images = bgImages.length > 0 ? bgImages : siteConfig.bgImages;
  const currentIndex = Math.max(0, images.indexOf(bgImage));
