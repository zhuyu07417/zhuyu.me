...
/** 从后端 API 读取站点配置，失败则回退到静态 siteConfig */
async function getSiteConfigFromAPI(): Promise<Record<string, string>> {
  try {
...
export async function generateMetadata(): Promise<Metadata> {
  const apiConfig = await getSiteConfigFromAPI();
  const title = apiConfig.site_title || siteConfig.title;
  const description = apiConfig.site_description || siteConfig.bio;
  return {
    title,
    description,
    icons: {
