...
def github_callback(
    ...
    redirect_url = f"{FRONTEND_ORIGIN}/auth/callback?token={token}&type=github&redirect={urllib.parse.quote(redirect_path)}"
...
def get_github_user_optional(request: Request, session: Session) -> GitHubUser | None:
    ...
    try:
        token = auth[7:]
        payload = jwt.decode(token, SECRET_KEY, algorithms=[ALGORITHM])
        # 只接受 github 类型的 token
        if payload.get("type") and payload.get("type") != "github":
            return None
        user_id = int(payload["sub"])
