...
def _comment_to_dict(session: Session, c: ChatterComment, include_ip: bool = False, fetch_replies: bool = False) -> dict:
    ...
    qq_user = session.get(QQUser, c.qq_user_id) if c.qq_user_id else None

    # 兼容前端：github_user 字段统一返回用户信息（GitHub 或 QQ）
    user_info = None
    if gh_user:
        user_info = {
            "id": gh_user.id,
...
def create_chatter_comment(
    ...
    qq_user: QQUser | None = None,
    ip: str = "",
) -> dict:
    if not github_user and not qq_user:
    ...
    comment = ChatterComment(
        ...
        github_user_id=github_user.id if github_user else None,
        qq_user_id=qq_user.id if qq_user else None,
        content=data.content,
        ip=ip,
    )
    session.add(comment)
