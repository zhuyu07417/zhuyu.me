...
from app.api.qq_auth_new import get_qq_user_optional
...
def create_chatter_comment(
    ...
    # 优先检查 GitHub 登录，再检查 QQ 登录
    gh_user = get_github_user_optional(request, session)
    qq_user = get_qq_user_optional(request, session)
    ip = request.headers.get("x-forwarded-for", "").split(",")[0].strip()
    if not ip:
        ip = request.headers.get("x-real-ip", "")
    if not ip:
        ip = request.client.host if request.client else ""
    return chatter_service.create_chatter_comment(session, data, github_user=gh_user, qq_user=qq_user, ip=ip)
