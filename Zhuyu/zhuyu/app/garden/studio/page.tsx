    ...
    <motion.div variants={fadeIn} initial="hidden" animate="show" className="h-screen flex flex-col">
      ...
      <div className="h-12 shrink-0 border-b border-slate-200/60 dark:border-white/5 bg-white/80 dark:bg-slate-900/60 backdrop-blur-xl flex items-center justify-between px-4">
        ...
        <div className="flex items-center gap-1">
          ...
          <button title="复制" onClick={duplicateSelected} className="w-8 h-8 rounded-lg hidden sm:flex items-center justify-center text-slate-400 hover:bg-slate-100 dark:hover:bg-slate-800 hover:text-slate-600 dark:hover:text-slate-300 transition-all">
      ...
      <div className="flex-1 flex overflow-hidden">
        ...
        <div ref={containerRef} className="flex-1 relative bg-[#e8e2da]">
          ...
          <div className="absolute bottom-4 left-1/2 -translate-x-1/2 text-[10px] text-slate-500 bg-white/80 backdrop-blur-sm px-4 py-1.5 rounded-full pointer-events-none whitespace-nowrap hidden md:block">
            点击选择 · 拖拽移动 · 右键旋转视角 · 左侧面板添加家具
          </div>
          <div className="absolute bottom-4 left-1/2 -translate-x-1/2 text-[10px] text-slate-500 bg-white/80 backdrop-blur-sm px-3 py-1.5 rounded-full pointer-events-none md:hidden">
            点击选择 · 双指操作
          </div>
        </div>
      ...
      {/* 移动端：添加家具浮动按钮 */}
      <button onClick={() => setShowFurniturePanel(true)}
        className="md:hidden fixed bottom-6 left-6 z-40 w-12 h-12 rounded-full bg-amber-600 text-white shadow-lg shadow-amber-600/30 flex items-center justify-center active:scale-90 transition-transform">
        <Armchair className="w-5 h-5" />
      </button>

      {/* 移动端：选中时属性按钮 */}
      {selectedData && (
        <button onClick={() => setShowPropertyPanel(true)}
          className="md:hidden fixed bottom-6 right-6 z-40 w-12 h-12 rounded-full bg-slate-800 dark:bg-slate-200 text-white dark:text-slate-900 shadow-lg flex items-center justify-center active:scale-90 transition-transform">
      ...
      {/* 移动端：家具库抽屉 */}
      {showFurniturePanel && (
        <div className="md:hidden fixed inset-0 z-50" onClick={() => setShowFurniturePanel(false)}>
          <div className="absolute inset-0 bg-black/40" />
          <div className="absolute bottom-0 left-0 right-0 bg-white dark:bg-slate-900 rounded-t-2xl max-h-[60vh] overflow-y-auto" onClick={e => e.stopPropagation()}>
            <div className="sticky top-0 bg-white dark:bg-slate-900 px-4 pt-4 pb-2 border-b border-slate-200/60 dark:border-white/5 flex items-center justify-between">
              <span className="text-xs font-semibold text-slate-800 dark:text-white">家具库</span>
              <button onClick={() => setShowFurniturePanel(false)} className="text-xs text-slate-400 hover:text-slate-600">关闭</button>
            </div>
            <div className="p-3">
              {categories.map((cat, ci) => {
                const Icon = (catIcons[cat.icon] || Armchair) as React.ComponentType<{ className?: string }>;
                ...
                return (
                  <div key={cat.name}>
                    <button onClick={() => setOpenCats(prev => ({ ...prev, [ci]: !isOpen }))}
                      className="flex items-center gap-2.5 w-full px-3 py-2.5 text-xs font-semibold text-slate-700 dark:text-slate-300">
                      <span className="w-7 h-7 rounded-lg bg-slate-100 dark:bg-slate-800 flex items-center justify-center">
                        <Icon className="w-3.5 h-3.5 text-amber-600" />
                      </span>
                      {cat.name}
                    ...
                    <div className={`overflow-hidden transition-all duration-300 ${isOpen ? "max-h-60" : "max-h-0"}`}>
                      {cat.items.map(item => (
                        <button key={item.type} onClick={() => { addFurniture(item.type); setShowFurniturePanel(false); }}
                          className="flex items-center gap-2 w-full pl-[44px] pr-4 py-2 text-xs text-slate-400 hover:text-slate-700 dark:hover:text-slate-200">
                          {item.name}
                        </button>
                      ))}
                    </div>
      ...
      {/* 移动端：属性抽屉 */}
      {showPropertyPanel && selectedData && (
        <div className="md:hidden fixed inset-0 z-50" onClick={() => setShowPropertyPanel(false)}>
          <div className="absolute inset-0 bg-black/40" />
          <div className="absolute bottom-0 left-0 right-0 bg-white dark:bg-slate-900 rounded-t-2xl max-h-[50vh] overflow-y-auto" onClick={e => e.stopPropagation()}>
            <div className="sticky top-0 bg-white dark:bg-slate-900 px-4 pt-4 pb-2 border-b border-slate-200/60 dark:border-white/5 flex items-center justify-between">
              <span className="text-xs font-semibold text-slate-800 dark:text-white">{selectedData.name}</span>
              <button onClick={() => setShowPropertyPanel(false)} className="text-xs text-slate-400 hover:text-slate-600">关闭</button>
            </div>
            <div className="p-4 space-y-4">
              <div className="space-y-2">
                <span className="text-[10px] font-semibold tracking-widest uppercase text-slate-400">位置</span>
                <PropSlider label="X" value={selectedData.posX} min={-3.8} max={3.8} step={0.05} unit="m" onChange={v => setPosX(parseFloat(v))} />
                <PropSlider label="Z" value={selectedData.posZ} min={-2.8} max={2.8} step={0.05} unit="m" onChange={v => setPosZ(parseFloat(v))} />
              </div>
              <div className="space-y-2">
                <span className="text-[10px] font-semibold tracking-widest uppercase text-slate-400">旋转</span>
                <PropSlider label="Y" value={selectedData.rotY} min={-180} max={180} step={1} unit="°" onChange={v => setRotY(parseFloat(v))} />
              </div>
              <div className="space-y-2">
                <span className="text-[10px] font-semibold tracking-widest uppercase text-slate-400">缩放</span>
                <PropSlider label="比例" value={selectedData.scale} min={0.5} max={2.0} step={0.05} unit="x" onChange={v => setScale(parseFloat(v))} />
              </div>
              {selPalette.length > 0 && (
                <div className="space-y-2">
                  <span className="text-[10px] font-semibold tracking-widest uppercase text-slate-400">颜色</span>
                  <div className="flex flex-wrap gap-2">
                    {selPalette.map(c => (
                      <button key={c.hex} title={c.name} onClick={() => setColor(c.hex)}
                        className={`w-8 h-8 rounded-full transition-all ${selectedData.color === c.hex ? "ring-2 ring-amber-500 ring-offset-2" : ""}`}
                        style={{ backgroundColor: c.hex }} />
                    ))}
                  </div>
                </div>
