...
// Garden 工具列表（与 layout.tsx navItems 保持同步）
const GARDEN_TOOLS = [
  "solar", "visitor", "fireworks", "fluid", "kaleidoscope",
  "sand", "life", "sorting", "color", "studio", "qrcode",
];
...
export default function GardenPage() {
  ...
  const statCards = [
    ...
    { icon: Gamepad2, label: "工具", value: GARDEN_TOOLS.length, suffix: " 个", color: "from-rose-500 to-rose-600", ring: "ring-rose-500/20" },
  ];

  return (
    <motion.div variants={container} initial="hidden" animate="show" className="px-3 py-4 md:p-6 lg:p-8 space-y-3 md:space-y-5 lg:space-y-6 max-w-full overflow-x-hidden">
      {/* ── top bar ── */}
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 md:gap-4">
        {statCards.map((s) => (
          ...
          >
            ...
            <p className="text-xl md:text-3xl font-bold text-slate-800 dark:text-white">
            ...
            <ResponsiveContainer width="100%" height="100%" minWidth={0}>
              <PieChart>
                ...
                <Legend
                  ...
                  iconType="circle"
                  iconSize={8}
                  formatter={(value: string) => <span className="text-[10px] md:text-xs text-slate-500 dark:text-slate-400">{value}</span>}
                />
              </PieChart>
            </ResponsiveContainer>
          </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
        ...
        <motion.div variants={fadeIn} className="bg-white/70 dark:bg-slate-800/50 backdrop-blur-md rounded-xl p-4 md:p-5 border border-slate-200/50 dark:border-white/5">
          ...
          <div className="grid grid-cols-3 md:grid-cols-2 gap-x-3 md:gap-x-4 gap-y-2">
            {techStack.map((t) => (
