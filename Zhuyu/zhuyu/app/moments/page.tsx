...
export default function MomentsPage() {
  ...
  const processedMoments = useMemo(() => {
    ...
    result.sort((a, b) => {
      ...
      const parseTime = (s: string) => {
        if (!s) return 0;
        // 如果是 UTC 时间（后端返回的格式），添加 Z 后缀确保一致解析
  ...
  useEffect(() => {
    ...
    function handleClickOutside(e: MouseEvent) {
      ...
      // 如果点击的不是说说卡片内部
      if (!target.closest('[data-moment-card]')) {
        setOnlyViewId(null);
      }
    }
    // 延迟绑定，避免收起时立即触发
  ...
  const masonryColumns = useMemo(() => {
    // 简单估算每条说说的高度（内容+图片+固定padding）
    const estimateHeight = (m: Moment) => {
      let h = 140; // 头部+底部栏+padding
      h += Math.ceil(m.content.length / 30) * 22; // 文字行
      if (m.images.length === 1) h += 200;
      else if (m.images.length > 1) h += Math.ceil(m.images.length / 3) * 100;
      return h;
    };
    // 按时间排序后分配到较短列
    const sorted = [...processedMoments];
    const cols: Moment[][] = [[], []];
    const heights = [0, 0];
    for (const m of sorted) {
      const target = heights[0] <= heights[1] ? 0 : 1;
      cols[target].push(m);
      heights[target] += estimateHeight(m);
    }
    return cols;
  }, [processedMoments]);
  ...
  function renderMomentCard(moment: Moment) {
    ...
    return (
      <motion.div
        ...
        data-moment-card className="flex flex-col bg-white/60 dark:bg-slate-800/50 backdrop-blur-xl rounded-3xl shadow-lg border border-white/40 dark:border-white/10 p-5 md:p-8 transition-shadow hover:shadow-xl overflow-hidden relative group w-full"
      >
        {/* 头部 */}
  ...
  return (
    <div className="w-[95%] md:w-[90%] max-w-6xl mx-auto py-6 md:py-10 mt-24 md:mt-28 relative z-10 flex-1 flex flex-col min-h-[85vh]">
      ...
      {processedMoments.length > 0 ? (
        <>
          {/* 手机端：单列 */}
          <div className="flex flex-col gap-5 md:hidden pb-32 w-full">
            <AnimatePresence mode="popLayout">
              {processedMoments.map((m) => renderMomentCard(m))}
            </AnimatePresence>
          </div>
          {/* 桌面端：双列瀑布流 */}
          <div className="hidden md:flex gap-8 pb-32 w-full items-start">
            <div className="flex-1 flex flex-col gap-8 w-full min-w-0">
              <AnimatePresence mode="popLayout">
                {masonryColumns[0].map((m) => renderMomentCard(m))}
              </AnimatePresence>
            </div>
            <div className="flex-1 flex flex-col gap-8 w-full min-w-0">
              <AnimatePresence mode="popLayout">
                {masonryColumns[1].map((m) => renderMomentCard(m))}
              </AnimatePresence>
            </div>
          </div>
        </>
