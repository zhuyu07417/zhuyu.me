...
export default function SearchBar() {
  ...
  const submit = (e: React.FormEvent) => {
    ...
    if (s) { router.push(`/timeline?keyword=${encodeURIComponent(s)}`); setOpen(false); }
  };
          ...
          <div className="absolute top-full left-0 right-0 mt-4 bg-white/90 dark:bg-slate-900/90 backdrop-blur-3xl border border-white/50 dark:border-slate-700/50 rounded-3xl shadow-2xl overflow-hidden max-h-[450px] overflow-y-auto z-20">
            {res.length>0 ? (
              <div className="flex flex-col py-3">
                {res.map(p=>(
                  <Link href={`/timeline/${p.slug}`} key={p.id} onClick={()=>setOpen(false)} className="px-6 py-5 hover:bg-indigo-50/80 dark:hover:bg-indigo-500/10 transition-colors border-b border-slate-100/50 dark:border-slate-800/50 last:border-0 flex flex-col gap-2">
...
{p.description && <p className="text-sm text-slate-500 dark:text-slate-400 line-clamp-2 leading-relaxed"><HL t={p.description} q={q.trim()} /></p>}
                ...
                {res.length>=8 && <Link href={`/timeline?keyword=${encodeURIComponent(q.trim())}`} onClick={()=>setOpen(false)} className="flex items-center justify-center gap-2 px-4 py-3 text-xs text-indigo-500 border-t border-slate-200/50 dark:border-slate-700/50 hover:bg-indigo-50/50 transition-colors">查看全部结果</Link>}
              </div>
            ) : res.length===0 ? (
